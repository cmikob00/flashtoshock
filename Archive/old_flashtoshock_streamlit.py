...
# NOTE: Physics and equations remain strictly identical to the verified model.
...
def plot_yield_results(d_meas, d_unc, t_meas, t_unc, W_nom, W_unc_range, rho, a, shock_model, burst_type, pdf_pages):
    ...
    st.pyplot(fig, use_container_width=False) # Render in Streamlit
    plt.close(fig)

def plot_fit(W_nom, rho, a, d_meas, t_meas, u, v, w, p, q):
    # convert yield in kt to joules
    W_nom_joules = W_nom * ktj

    # define characteristic length and time scales
    lc = (W_nom_joules / (rho * a**2.))**(1. / 3.)  # characteristic length scale
...
if st.button("Run Analysis", type="primary"):
    ...
    # Text accumulator to mimic the Tkinter terminal
    log_output = []
    def log(msg):
    ...
    try:
        ...
        # Display area for logs and plots
        ...
        with results_container:
            if mode == "Solve for Yield (Given Distance)":
                ...
                log(f"Atmosphere: T={T_k:.1f} K, rho={rho:.3f} kg/m3, a={a:.1f} m/s")
                ...
                log("Integrating atmospheric path...")
